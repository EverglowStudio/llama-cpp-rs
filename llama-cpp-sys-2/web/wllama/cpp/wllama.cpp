...
#define WLLAMA_ACTION(name) \
  ...
  { \
    const bool is_mtmd_eval_action = action == "mtmd_eval_chunks"; \
    if (is_mtmd_eval_action) \
    { \
      wllama_native_breadcrumb("wllama_action before action_mtmd_eval_chunks"); \
    } \
    auto res = action_##name(app, req_raw); \
    if (is_mtmd_eval_action) \
    { \
      wllama_native_breadcrumb("wllama_action after action_mtmd_eval_chunks before serialize"); \
    } \
    res.handler.serialize(output_buffer); \
    if (is_mtmd_eval_action) \
    { \
      wllama_native_breadcrumb("wllama_action after action_mtmd_eval_chunks serialize"); \
    } \
  }

static const char *const WLLAMA_MTMD_ACTION_NAME_ANCHORS[] = {
    "load_mmproj_or_mtmd_context",
    "load_media_bytes",
    "mtmd_tokenize",
    "mtmd_eval_chunks",
};

static volatile int wllama_mtmd_action_anchor_index = 0;

static void keep_wllama_mtmd_action_name_anchors()
{
  const int anchor_count =
      static_cast<int>(sizeof(WLLAMA_MTMD_ACTION_NAME_ANCHORS) / sizeof(WLLAMA_MTMD_ACTION_NAME_ANCHORS[0]));
  const int anchor_index = wllama_mtmd_action_anchor_index % anchor_count;
  if (WLLAMA_MTMD_ACTION_NAME_ANCHORS[anchor_index][0] == '\0')
  {
    abort();
  }
}
...
{
  printStr(GGML_LOG_LEVEL_ERROR, text);
}
...
extern "C" const char *wllama_start()
{
  try
  {
    keep_wllama_mtmd_action_name_anchors();
    llama_backend_init();
...
extern "C" const char *wllama_action(const char *name, const char *req_raw)
{
  try
  {
    ...
    WLLAMA_ACTION(load)
    WLLAMA_ACTION(load_mmproj_or_mtmd_context)
    WLLAMA_ACTION(load_media_bytes)
    WLLAMA_ACTION(mtmd_tokenize)
    WLLAMA_ACTION(mtmd_eval_chunks)
  ...
  {
    printStr(GGML_LOG_LEVEL_ERROR, (std::string("wllama_action threw std::string: ") + e).c_str());
    return nullptr;
  }
  catch (const char *e)
  {
    printStr(GGML_LOG_LEVEL_ERROR, (std::string("wllama_action threw const char *: ") + (e == nullptr ? "<null>" : e)).c_str());
    return nullptr;
  }
  catch (...)
  {
    printStr(GGML_LOG_LEVEL_ERROR, "wllama_action threw unknown exception");
    return nullptr;
  }
}
